...
module.exports={
    ...
    async fetchContentOfFile(req,res)
    {
     ...
     generatePrompt(code,filename) {
        ...
        return `Act as a senior developer and simulate a situation where you are reviewing a PR. Make comments that highlight potential bugs and tell me how can I resolve that bug and return me a category for the comment telling me wether that issue is either of the three options in critcality : [High, Medium, Low] then return me a json which has an array of objects in the form , {result : [{fileName,lineNumber,comment,solutionToFixThatComment,category}]}. If you spot a piece of code that can be written in an optimized manner, please highlight that and send a potential solution for it like I explained previously for the bug spotting. Count the number of lines properly and return the right number on which you want to comment.Be extra sure for the line number and count them properly. Check the code thoroughly for errors including but not limited to : Syntax Errors,Logic Errors,Compilation Errors,Runtime Errors and Arithmetic Errors. 
        
        An example array of objects that you can send is like this : 
        ...
        Point out things like there should not be any code that is not being used in the file. Take care of other code readability and formatting issues.
        
        Only return me a json after doing the code review and nothing else. Do not return any text other than text than the final array and return the text in a form that if I do JSON.parse then I get a perfectly working json object of the exact format as specified above.
